    ...
    while not done:
        if 'success' in data and data['success'] == 1:  # if the query was successful
            if 'reviews' in data and len(data['reviews']) > 0:  # if we received reviews
                ...
                params['start_offset'] += data['query_summary'][
                    'num_reviews']  # increase the start offset by the number of reviews received in this response
                if printProgress:
            ...
            raise Exception('Steam Web API returned null. Rate limit may be exceeded.')
        elif 'success' in data and data['success'] == 2:
            done = True
            if printProgress:
                print('success=2')
            ...
            raise Exception('Steam Web API appreviews request was unsuccessful.')

    if printProgress:
        print("Apps: " + appid + " Found all reviews.")
    return results